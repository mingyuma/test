...
public class ImageSettingUtil {
	...
	private static String uploadImageUrl=ConnectUtil.UploadImage+"?";
	
	public interface ImageUploadDelegate{
		//上传过程中需要给更新的进度
		public void setUploadProgress(int index,double x);//index 表示第几个进度条，x表示进度,当x为负数时，表示上传出现问题
		public void getRecodeFromServer(int index,String reCode);
	}
	//上传图片,后台为struct2服务器
	public static void uploadImage(ImageUploadDelegate delegate,int index,PostParameter[] postParams,ImageVo imageVo,InputStream in){
		
		String uploadUrl=uploadImageUrl+ConnectUtil.encodeParameters(postParams);
		...
		//out.writeBytes("Content-Disposition: form-data; name=\"file\"; filename=\"" +imageVo.getImageName()+ "\"" + end);
		//out.writeBytes("Content-Type: application/octet-stream; charset=utf-8"  
        //      + end);  
		//out.writeBytes(end);
		Log.i("liuhaoxian", "upload start");
		//上传图片
		//InputStream in=ImageSettingUtil.compressJPG(imageVo.getImagePath());
		//更新进度
		byte[] buffer =new byte[1024*8];
		int total=in.available();
		Log.i("liuhaoxian", "total size="+total);
		int length=0;
		int count=0;
		while((length=in.read(buffer))!=-1){
			...
			Log.i("liuhaoxian", "uploading---->"+(1.0*count/total));
		}
		//out.writeBytes(end);
		...
		Log.i("liuhaoxian", ""+System.currentTimeMillis());
		//等待服务器返回结果
		...
		if(result!=null&&result.contains("SUCCESS")){
			delegate.getRecodeFromServer(index, "SUCCESS");
		}
		else{
			delegate.getRecodeFromServer(index, "FALSE");
		}
		}
		catch(Exception e){
			e.printStackTrace();
